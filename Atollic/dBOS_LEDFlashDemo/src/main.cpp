 ...
 * @date 27/05/2015
 *
 * \brief Simple LED flasher using task waiting on timer object.
 *
 */
#include "Device.h"
#include "ProjUARTs.h"
...
int main(void){
	...
	GPIOC->CRH &= ~(GPIO_CRH_MODE12_0 | GPIO_CRH_MODE12_1 | GPIO_CRH_CNF12_0 | GPIO_CRH_CNF12_1);
	GPIOC->CRH |= GPIO_CRH_MODE12_1;	//2MHz Push-pull output.
...
static void LEDFlashTest_IdleTask(void * D){
...
static void LEDFlashTest_TaskOneFunc(void * D){
	...
	dBOS_TIMERINTERFACE_P_t pTimer = pK->CreateTimer("LED Timer", TIMER_COUNTDOWN_FLAG | TIMER_RELOAD_FLAG, 500, 500, PRIORITYLIST);
	...
	while(1){
		...
		pK->Object_Wait(pTimer, &SignalData, TIMEWAITFOREVER);

		if(LED == 1U){
			LED = 0U;
			Per_SetBit_BB( (uint32_t) &(GPIOC->BSRR), 12);
		}
		else{
			LED = 1U;
			Per_SetBit_BB( (uint32_t) &(GPIOC->BRR), 12);
		}
...
void LEDFlashTest(void){
	...
	TaskOne = pK->CreateTask("Task One", 10, 196, LEDFlashTest_TaskOneFunc, NULLPTR);

	pK->Start("Idle Task", 128, LEDFlashTest_IdleTask);
